...
\date{Due by email 5PM Friday, Oct 30, \the\year{} \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\intermediatesubproblem{Let $X \sim \exponential{\lambda}$. Find the PDF of $Y = g(X) = ke^X$. This will be a brand name r.v., so mark it so and include its parameter values.}\spc{6}
